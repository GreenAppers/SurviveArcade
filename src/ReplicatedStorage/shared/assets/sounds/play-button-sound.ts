...
export const playButtonDown = throttle(
  (variant: ButtonSoundVariant = 'default') => {
    if (
      variant === 'none' ||
    ...
    return playSound(
      variant === 'default' ? sounds.button_down : sounds.button_down_alt,
      {
        volume: 0.25,
      },
    )
...
export async function playButtonUp(variant: ButtonSoundVariant = 'default') {
  ...
  return playSound(
    variant === 'default' ? sounds.button_up : sounds.button_up_alt,
    {
      volume: 0.25,
    },
  )
